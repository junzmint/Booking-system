...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Khởi tạo một biến để chứa thông điệp lỗi
      let errorMessage = "An error occurred while registering."; // Phương án dự phòng

      // Kiểm tra lỗi liên quan đến username
      if (error.response?.data?.message?.keyValue?.username) {
        errorMessage = `Username "${error.response.data.message.keyValue.username}" is already taken.`;
      }

      // Kiểm tra lỗi liên quan đến email
      else if (error.response?.data?.message?.keyValue?.email) {
        errorMessage = `Email "${error.response.data.message.keyValue.email}" is already in use.`;
      }
